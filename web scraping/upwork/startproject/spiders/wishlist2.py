...
class Wishlist2Spider(scrapy.Spider):
    name = 'wishlist2'
    ...
    def start_requests(self):
        ...
        # create HtmlResponse object from the current URL and the webdriver page source
        response = HtmlResponse(url=self.driver.current_url, body=self.driver.page_source.encode('utf-8'), encoding='utf-8')

        yield scrapy.Request(url=response.url, callback=self.parse, body=response.body)

        # yield SeleniumRequest(url= self.driver.current_url, callback=self.parse)
        # create a scrapy response object from the webdriver's HTML content
