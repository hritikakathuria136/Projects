...
class Web2Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        rows = response.xpath('//div[@class="card container__item container__item--type- __item __item--type- "][contains(text(), "NAME")]')
        for row in rows:
            # Extract the relevant data from the row
            data = {
                'uri': row.xpath('@data-uri').get(),
                'class': row.xpath('@class').get(),
                'headline': row.xpath('h2/text()').get().strip(),
                # add more data to extract as needed
            }
            yield data
